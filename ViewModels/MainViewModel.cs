...
public ObservableCollection<RssGroup> GroupedRssItems
{
    ...
    set => SetProperty(ref _groupedRssItems, value);
        ...
        public ICommand FullGlobalSyncCommand => new Command(async () =>


        {
            // Si déjà en cours, on sort immédiatement
            if (IsSyncing) return;

            await ExecuteFullGlobalSyncCommand();
        },
        // Cette condition désactive automatiquement le bouton dans l'UI
        () => !IsSyncing);
        ...
        public ICommand StopSyncCommand => new Command(() =>
        {
            _syncCts?.Cancel(); // C'est cette ligne qui arrête réellement les tâches
            IsBusy = false;
        });
        ...
        {
            // 1. Protection contre les lancements multiples (Sécurité basse couche)
            if (!_syncSemaphore.Wait(0)) return;

            // 2. Gestion propre de l'annulation
            ...
            try
            {
                ...
                foreach (var playlist in playlistsList)
                {
                    ...
                    try
                    {
                        // PROPAGATION DU TOKEN : Arrêt immédiat si le service est annulé
                        await _rssService.SynchronizeSitesInPlaylistAsync(playlist.Id, token);
                    }
                    catch (OperationCanceledException) { break; } // Sortir de la boucle si annulé
                    catch (Exception ex) { Debug.WriteLine($"[SYNC ERR] {ex.Message}"); }

                    // Gestion des icônes avec support du token
                    var sites = await _dbService.GetSitesByPlaylistIdAsync(playlist.Id);
                    foreach (var site in sites)
                    {
                        if (token.IsCancellationRequested) break;

                        // Icône : Mise à jour URL si nécessaire
                        ...
                        // Icône : Téléchargement local avec token
                        if (!string.IsNullOrEmpty(site.IconUrl) && string.IsNullOrEmpty(site.LocalIconPath))
                        {
                            string localPath = await _rssService.DownloadAndSaveIconAsync(site.IconUrl, site.Name, token);
                ...
                {
                    SyncStatus = "Synchronisation annulée.";
                }
                else
                {
            ...
            finally
            {
                ...
                // Libération du verrou et nettoyage du token
