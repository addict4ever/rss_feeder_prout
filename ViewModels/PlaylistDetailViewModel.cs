...
namespace Rss_feeder_prout.ViewModels
{
    ...
    public class PlaylistDetailViewModel : BaseViewModel
    {
        ...
        private async Task ExecuteAddSiteCommand()
        {
            ...
            // 1. Vérification basique de format
            ...
            // 3. VÉRIFICATION RÉSEAU : Est-ce un vrai flux RSS ?
            IsBusy = true;
            try
            {
                // On tente de lire le flux. Si l'URL n'est pas un flux, cela lancera une exception.
                var feed = await CodeHollow.FeedReader.FeedReader.ReadAsync(url);

                // Si on arrive ici, le flux est valide
                string siteName = !string.IsNullOrWhiteSpace(feed.Title) ? feed.Title : new Uri(url).Host;
                ...
                var newSite = new FeedSite
                {
                    ...
                    IconUrl = feed.ImageUrl // Optionnel : récupère l'icône du flux si disponible
                };
            ...
            {
                Debug.WriteLine($"[RSS ERROR] Erreur lors de la validation du flux : {ex.Message}");
                await Shell.Current.DisplayAlert("Erreur", "Impossible de valider ce flux RSS. Vérifiez l'URL.", "OK");
            }
            finally
            {
                IsBusy = false;
